...
export const AnalysisPanel: React.FC<AnalysisPanelProps> = ({
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700/50">
          ...
          <div className="text-xs text-slate-500 mt-1">个群组</div>
        </div>
        ...
        <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700/50">
          ...
          <div className="text-xs text-slate-500 mt-1">个关键实体</div>
        </div>
      </div>
  ...
  const renderSuggestions = () => (
    <div className="space-y-2">
      ...
      {(!investigationAnalysis || investigationAnalysis.prioritizedSuggestions.length === 0) && (
        <div className="py-12 text-center">
          <CheckCircle className="w-12 h-12 text-green-500 mx-auto mb-3 opacity-50" />
          <div className="text-sm text-slate-400">所有节点完整性良好</div>
          <div className="text-xs text-slate-500 mt-1">没有需要优先处理的调查建议</div>
        </div>
      )}
    </div>
  );
      ...
      >
        ...
        <div className="flex border-b border-slate-800 bg-slate-950/30">
          {[
            ...
            { key: 'suggestions', label: '调查建议', icon: AlertTriangle },
          ].map(tab => (
            <button
              key={tab.key}
            ...
            >
              ...
              {tab.label}
              {tab.key === 'suggestions' && investigationAnalysis && investigationAnalysis.prioritizedSuggestions.length > 0 && (
                <span className="px-1.5 py-0.5 text-[10px] bg-orange-500/20 text-orange-400 rounded-full">
                  {investigationAnalysis.prioritizedSuggestions.length}
                </span>
              )}
            </button>
          ))}
